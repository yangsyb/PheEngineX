...
namespace Phe
{
	...
	PStaticMesh::PStaticMesh(std::string Name, std::vector<float> Vertices, std::vector<UINT16> Indices, std::vector<float> Normals, std::vector<float> Tangents, std::vector<float> TangentYs, std::vector<float> UVs) : PMeshName(Name), PVertices(Vertices), PIndices(Indices), PTangents(Tangents), PUVs(UVs), PNormals(Normals), PTangentYs(TangentYs)
	{
