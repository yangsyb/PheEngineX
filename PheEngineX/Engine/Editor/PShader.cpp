...
namespace Phe
{
	PShader::PShader(const std::string ShaderName, const std::wstring FilePath, std::string VS, std::string PS, P_RasterizerDesc Raster, P_BlendState Blend, P_DepthStencilState DepthStencil) : PFilePath(FilePath), PVSEntry(VS), PPSEntry(PS), PShaderName(ShaderName)
		, RasterizerDesc(Raster), BlendState(Blend), DepthStencilState(DepthStencil)
	{
